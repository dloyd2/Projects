...
//
	...
	switch(curr_state)
	{
		...
		case STOPPED:
			SendContSig();
			break;
		default:		//should not reach default, only exists to catch weird errors
			break;
	}
	SetAlarm();
}
...
//
	if(pbArray[pbCounter].status == RUNNING){SendStopSig();}	//if child completed then dont stop it, would change the status in pbArray from EXITED to STOPPED and it will get rescheduled. Don't need to send a stop signal since process is done, it is already stopped
	do
	{
		pbCounter = (pbCounter+1)%numCommands;
	}
	while(pbArray[pbCounter].status == RUNNING || pbArray[pbCounter].status == EXITED);	//find a process that is waiting to run
	RunProcess();
}
